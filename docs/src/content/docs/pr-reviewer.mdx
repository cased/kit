...
# 5. Review local changes before creating PRs
kit review --local-diff main..feature-branch

# 6. Test without posting (dry run with full formatting)
kit review --dry-run https://github.com/owner/repo/pull/123

# 7. Use custom context profiles for organization standards
...
# 9. Quick PR summaries for triage (5-10x cheaper)
kit summarize https://github.com/owner/repo/pull/123

# 10. Add summary to PR description for team visibility
...
Review your changes **before creating PRs** with kit's local diff analysis. Get the same repository intelligence and AI analysis on local branches, uncommitted changes, or any git diff.

```bash
# Review branch changes before creating PR
kit review --local-diff main..feature-branch
...
# Pipe directly to Claude Code for implementation
kit review --local-diff main..feature-branch | \
  claude "implement these code review suggestions"

# Use agentic mode for complex local changes
kit review --local-diff main..feature-branch --agentic --agentic-turns 10
...
- **Same Repository Intelligence**: Full symbol extraction, dependency analysis, and cross-file impact
- **Plain Output by Default**: Perfect for piping to other tools (use `--dry-run` for formatted preview)
- **All Analysis Modes**: Standard and agentic analysis both supported
- **Compatible Options**: Works with `--priority`, `--profile`, `--model`, and all other options
- **No Posting**: Local diffs never post to GitHub - always local analysis only
...
# 2. Make changes and get AI review
kit review --local-diff main..new-feature

# 3. Address feedback, then create PR
git push origin new-feature
gh pr create

# 4. Get final AI review on the actual PR
kit review https://github.com/owner/repo/pull/123
```
