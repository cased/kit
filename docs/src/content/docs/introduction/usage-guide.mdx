...
Operations scoped to specific files or directories are fast:

```python
# Fast: single file (~30ms)
symbols = repo.extract_symbols("src/my_module.py")

# Fast: scoped to directory
tree = repo.get_file_tree("src/")
```

Bulk operations that scan the entire repository are slower on large codebases:

```python
# Slower: parses ALL files (can take seconds on large repos)
all_symbols = repo.extract_symbols()  # no file argument
index = repo.build_index()
usages = repo.find_symbol_usages("MyClass")
```

For most use cases, scope to specific files or directories rather than scanning everything.
...
For microservices, monorepos, or multi-project codebases, use `MultiRepo` to analyze multiple repositories together:
...
user_models = repos.find_symbol("UserModel", symbol_type="class")

# Audit dependencies (Python, JS, Rust, Go)
deps = repos.audit_dependencies()

# Get summaries of all repos
...
```bash
kit multi search ~/frontend ~/backend -q "TODO"
kit multi deps ~/service-a ~/service-b ~/service-c
kit multi summary ~/project-*
```

See the [MultiRepo API reference](/api/multi-repo) for complete documentation and use cases.
