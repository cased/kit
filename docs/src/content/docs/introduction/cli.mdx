...
Search through source code of popular packages using regex patterns (requires Chroma API key).
...
# Search for decorators in Flask
kit package-search-grep flask "@app\\.route" -f "*.py"

# Find FFT functions in numpy
kit package-search-grep numpy "def.*fft" --json
```
...
kit package-search-hybrid numpy "fast fourier transform" -r "def.*fft"
```
...
# Read specific lines
kit package-search-read flask "flask/app.py" -s 100 -e 200
```
