...
AI-powered GitHub pull request reviewer that provides comprehensive code analysis with full repository context. The reviewer clones repositories, analyzes symbol relationships, and generates intelligent reviews using Claude or GPT-4. Supports local branch comparisons for reviewing changes before creating PRs.

```bash
kit review <pr-url> [OPTIONS]
kit review --local-diff <diff-spec> [OPTIONS]
```

**Options:**
- `--local-diff, -l <diff-spec>`: Local branch comparison (e.g., 'main..feature-branch' or 'HEAD~1..HEAD')
...
```bash
# Review GitHub PR and post comment
kit review https://github.com/owner/repo/pull/123

# Review local branch changes before creating PR
kit review --local-diff main..feature-branch

# Review recent commits
kit review --local-diff HEAD~3..HEAD

# Review uncommitted changes (working directory vs HEAD)
kit review --local-diff HEAD

# Dry run (formatted preview without posting)
kit review --dry-run https://github.com/owner/repo/pull/123
kit review --local-diff main..feature-branch --dry-run

# Clean output for piping to other tools
kit review --plain https://github.com/owner/repo/pull/123
kit review --local-diff main..feature-branch  # defaults to plain output

# Override model for specific review
kit review --model gpt-4.1-nano https://github.com/owner/repo/pull/123
kit review --local-diff main..feature-branch -m claude-3-5-haiku-20241022

# Pipe local review to Claude Code for implementation
kit review --local-diff main..feature-branch | \
  claude "Implement these code review suggestions"

# Use agentic mode for complex changes
kit review --local-diff main..feature-branch --agentic --agentic-turns 15

# Focus on high-priority issues only
kit review --local-diff HEAD~5..HEAD --priority=high
...
**Local Diff Features:**
- **Branch Comparisons**: Compare any two git refs (`base..head`)
- **Commit Ranges**: Review multiple commits (`HEAD~3..HEAD`)
- **Full Repository Analysis**: Uses kit's symbol extraction and dependency analysis
- **Plain Output by Default**: Local diffs default to plain output (use `--dry-run` for formatted preview)
- **Agentic Analysis**: Multi-turn analysis for complex local changes
- **Priority Filtering**: Focus on high/medium/low priority issues
