...
Perform literal grep search on repository files using system grep.
...
kit grep /path/to/repo "class" --include "*.py"

# Exclude test files
kit grep /path/to/repo "import" --exclude "*test*"

# Limit results and save to file
kit grep /path/to/repo "error" --max-results 50 --output errors.json

# Complex filtering
kit grep /path/to/repo "api_key" --include "*.py" --exclude "*test*" --ignore-case
```
