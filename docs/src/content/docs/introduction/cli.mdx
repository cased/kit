...
# Use specific model
kit summarize --model claude-haiku-4-5 https://github.com/owner/repo/pull/123
...
#### `kit package-search-grep`

Search through source code of popular packages using regex patterns (requires Chroma API key). Outputs in plain grep-style format by default for pipe-friendly scripting.

```bash
kit package-search-grep <package> <pattern> [OPTIONS]
```

**Options:**
- `--max-results, -m <count>`: Maximum number of results (default: 20)
- `--file-pattern, -f <pattern>`: Filter files by glob pattern (e.g., '*.py')
- `--case-sensitive, -c` / `--ignore-case, -i`: Case sensitivity
- `--json, -j`: Output as JSON with full metadata
- `--formatted, -F`: Output with formatting and emojis (default is plain like grep)

**Examples:**

```bash
# Find all async functions in FastAPI (plain grep-style output)
kit package-search-grep fastapi "async def \w+" --max-results 10

# Search for decorators in Flask with pipe-friendly output
kit package-search-grep flask "@app\\.route" -f "*.py" | head -5

# Get structured JSON output for programmatic processing
kit package-search-grep numpy "def.*fft" --json | jq '.[] | .file_path'

# Pretty formatted output with emojis
kit package-search-grep django "class.*Model" --formatted
```

#### `kit package-search-hybrid`

Perform semantic search in package source code with optional regex filtering.

```bash
kit package-search-hybrid <package> <query> [OPTIONS]
```

**Options:**
- `--max-results, -m <count>`: Maximum results (default: 20)
- `--regex-filter, -r <pattern>`: Filter with regex pattern
- `--file-pattern, -f <pattern>`: Filter files by glob pattern
- `--json, -j`: Output as JSON with full metadata
- `--plain, -p`: Output plain text without formatting

**Examples:**

```bash
# Find HTTP functionality in requests
kit package-search-hybrid requests "HTTP connection pooling and retry logic"

# Search with regex filter and JSON output
kit package-search-hybrid numpy "fast fourier transform" -r "def.*fft" --json

# Plain output for piping
kit package-search-hybrid flask "route handling" --plain | head -10
```

#### `kit package-search-read`

Read specific files from package source code.

```bash
kit package-search-read <package> <file-path> [OPTIONS]
```

**Options:**
- `--start-line, -s <line>`: Starting line number
- `--end-line, -e <line>`: Ending line number
...
```bash
# Read entire file with header
kit package-search-read requests "requests/models.py"

# Read specific lines in plain format
kit package-search-read flask "flask/app.py" -s 100 -e 200 --plain

# Get JSON output with metadata
kit package-search-read numpy "numpy/__init__.py" --json
```

**Configuration:**
Set the `CHROMA_PACKAGE_SEARCH_API_KEY` environment variable or get a key from [Chroma Cloud](https://cloud.trychroma.com).
...
kit review -m claude-haiku-4-5 https://github.com/owner/repo/pull/123
...
**Frontier Tier ($3-15/MTok)**
- `claude-opus-4-20250514`: Latest flagship, superior complex reasoning
- `claude-sonnet-4-5`: High-performance with exceptional reasoning (recommended)
- `gpt-5.1`: Latest OpenAI model with adaptive reasoning

**Balanced Tier ($0.10-1/MTok)**
- `gpt-4.1`: Good balance of performance and cost
- `gpt-4.1-mini`: Budget-friendly with solid capability
- `claude-haiku-4-5`: Ultra-fast and economical

**Budget Tier ($0.0015-0.01/MTok)**
- `gpt-4.1-nano`: Ultra-budget, high-volume use
- `gemini-2.5-flash`: Excellent value, fast
