...
2.  **Build or Load the Index**
    ...
    if not os.path.exists(INDEX_DB_PATH):
        ...
        indexer.build_index(level="symbol", file_extensions=[".py", ".js", ".md"], force=True)
        ...
        for res in search_results:
            try:
                ...
                if res.get('level') == 'symbol':
                    ...
                    if symbol_name:
                        code_content = repo.get_symbol_text(res['file_path'], symbol_name)
                        chunk_identifier = f"{res['file_path']}::{symbol_name}"
                    else:
                        logger.warning(f"Symbol level result missing symbol_name: {res}")
                        continue
                ...
                if not code_content.strip():
                    logger.warning(f"No content retrieved for {chunk_identifier}, skipping.")
                    continue

                context_chunks.append({
                    "code": code_content,
                    "identifier": chunk_identifier # Use a generic 'identifier' key
                })
            except FileNotFoundError:
                print(f"Warning: Could not retrieve content for {res['file_path']}")
            except ValueError as ve: # From get_symbol_text if symbol not found
                print(f"Warning: Could not retrieve symbol text for {res.get('symbol_name')} in {res['file_path']}: {ve}")
...
*   **Hybrid Search**: Combine file-level and symbol-level search results for broader context or more precise answers.
