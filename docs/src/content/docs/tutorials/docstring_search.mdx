...
indexer.build()          # writes to repo_path/.kit_cache/docstring_db by default (vector DB and cache metadata)
```

The first run will take time depending on repo size and LLM latency.
Summaries and file change metadata are cached (by default in a `meta.json` file within the `persist_dir`), 
