...
kit review -m claude-sonnet-4-5 https://github.com/owner/repo/pull/123
```
...
# Latest models (GPT-5.1 and O3 series)
gpt-5.1                  # Latest model with adaptive reasoning
gpt-5                    # GPT-5 multimodal model
...
claude-sonnet-4-5   # Excellent balance
...
# Standard models
grok-4                      # Advanced reasoning and coding
grok-3                      # Strong general purpose model
grok-3-mini                 # Faster, budget-friendly option
```

<Aside type="note">
**Using Grok**: Set `llm.provider: openai` and `llm.api_base_url: https://api.x.ai/v1` in your config file, then use any Grok model name. Requires credits at https://console.x.ai
</Aside>
