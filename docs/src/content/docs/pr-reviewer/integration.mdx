...
REVIEW=$(kit review -p --model claude-haiku-4-5 <pr-url>)
...
- name: AI Review with Multi-Stage Processing
  run: |
    ...
    REVIEW=$(kit review -p --model claude-haiku-4-5 ${{ github.event.pull_request.html_url }})
...
HIGH_ISSUES=$(kit review -p --model gpt-4.1-mini --priority=high <pr-url>)
