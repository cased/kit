...
For production deployments, you can restrict which repository domains the API will accept by setting the `KIT_ALLOWED_REPO_DOMAINS` environment variable. This prevents Server-Side Request Forgery (SSRF) attacks where untrusted URLs could be used to access internal resources.

**Configuration:**

```bash
# Allow only specific Git hosting providers
...
- **Configured**: Only URLs from the specified domains are accepted
- **Local paths**: Always allowed regardless of allowlist configuration
- **Blocked URLs**: Return HTTP 403 with a helpful error message listing allowed domains

**Example error response** when allowlist is configured:
...
**Use cases:**
- Production deployments where only trusted Git hosts should be accessed
- Preventing access to cloud metadata endpoints (169.254.169.254, metadata.google.internal)
- Blocking localhost and private IP ranges
- Defense-in-depth against SSRF attacks
