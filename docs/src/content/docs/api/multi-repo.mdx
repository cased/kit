...
Identify version inconsistencies and outdated packages:

```python
repos = MultiRepo(["~/code/service-a", "~/code/service-b", "~/code/service-c"])
audit = repos.audit_dependencies()

# Find services using different versions of the same package
for repo, deps in audit.items():
    if "python" in deps and "requests" in deps["python"]:
        print(f"{repo}: requests=={deps['python']['requests']}")
```
...
repos = MultiRepo([
    "/path/to/repo-a",
    "/path/to/repo-b",
    "~/code/repo-c",  # Tilde expansion supported
])

# Access individual repos
print(repos.names)  # ['repo-a', 'repo-b', 'repo-c']
...
- `paths`: List of paths to repositories (local paths, tilde-expanded paths)
- `names`: Optional dict mapping resolved path strings to friendly names. If not provided, directory names are used automatically
