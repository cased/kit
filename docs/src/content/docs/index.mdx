...
<<<<<<< HEAD
  Use kit to build code reviewers, code generators, even full-fledged IDEs, all enriched with the right code context. 
  MIT-licensed.
  
  kit shines for getting **precise, accurate, and relevant context** to 
  LLMs, so you can build better tools: whether internal, public, or commercial.
=======
    Use kit to build code reviewers, code generators, even full-fledged IDEs, all enriched with the right code context.
...
>>>>>>> a597ec1 (Update landing page hero section with responsive layout and styling)
</div>
