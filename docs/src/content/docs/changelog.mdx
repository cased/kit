...
- **Local Branch Review Support**: Review local changes before creating PRs
  - New `--local-diff` option for `kit review` command enables local branch comparisons  
  - Support for any git diff specification (`main..feature-branch`, `HEAD~3..HEAD`)
  - Plain output by default (perfect for piping to CLI tools like Claude Code)
  - Compatible with all existing options: `--agentic`, `--priority`, `--profile`, `--model`
  - Both standard and agentic analysis modes supported for complex local changes
