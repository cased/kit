...
- **Duplicate TypeScript / TSX Symbols**: Resolved an issue where some symbols could be recorded twice in `symbols.json` depending on tree-sitter queries. 
  - The `TreeSitterSymbolExtractor` now removes duplicate symbols based on `(name, type, start_line, end_line)` and a dedicated regression test suite was added.
  - Coverage: `tests/test_typescript_duplicate_symbols.py`, `tests/test_tsx_duplicate_symbols.py`.
