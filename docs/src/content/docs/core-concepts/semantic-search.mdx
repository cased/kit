...
Kit supports [Chroma Cloud](https://trychroma.com), a fully managed vector database service, as an alternative to local ChromaDB storage. This enables better scalability, team collaboration, and eliminates local storage constraints for large codebases.
...
- Python 3.10+
- Chroma Cloud account (sign up at [https://trychroma.com/signup](https://trychroma.com/signup))
...
**Note:** The tenant must be a valid UUID. Kit will validate the format and provide a clear error if an invalid tenant is provided. Find your tenant UUID in your [Chroma Cloud dashboard](https://trychroma.com).
...
Large repositories can exceed Chroma's API batch limits. Kit automatically
detects the backend cap and defaults to batches of up to 2000 items, but you can
override this with the `KIT_CHROMA_BATCH_SIZE` environment variable:

```bash
export KIT_CHROMA_BATCH_SIZE=1000  # send at most 1000 embeddings per add()
```

This applies to both local and cloud Chroma backends.
