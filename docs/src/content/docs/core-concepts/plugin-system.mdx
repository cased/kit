...
Haskell is supported via the official tree-sitter grammar. Queries capture the module header, functions (including lambda-binds), and common type-level declarations.

```scheme
; queries/haskell/tags.scm (excerpt)

; Module header (avoids imports)
(haskell
  (header (module) @name) @definition.module)

; Named function and lambda-bind
(decl/function name: (variable) @name) @definition.function
(decl/bind name: (variable) @name (match expression: (expression/lambda))) @definition.function
(decl/bind name: (variable) @name (#eq? @name "main")) @definition.function

; Type-level declarations
(data_type  name: (name) @name) @definition.data
(newtype    name: (name) @name) @definition.newtype
(type_synomym name: (name) @name) @definition.type
(class      name: (name) @name) @definition.class
(type_family name: (name) @name) @definition.type_family
(instance   name: (name) @name) @definition.instance
```

Then extract symbols from `.hs` files just like other languages:
...
symbols = TreeSitterSymbolExtractor.extract_symbols(".hs", code)
for s in symbols:
    print(s["type"], s["name"])  # module, function, data, newtype, type, class, type_family, instance
```
