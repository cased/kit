---
title: Choosing a Search Approach
---
...
`extract_symbols()` parses Python ASTs to list functions, classes, variables –
useful for building nav trees or refactoring tools.
...
`VectorSearcher` chunks code (symbols or lines) → embeds chunks → stores in
Chroma.  Good when wording of the query is *similar* to the code.

### 5. Docstring vector search

`DocstringIndexer` first *summarises* each file, then embeds the summary.  The
