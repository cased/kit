...
1.  **Install Extra Dependencies:**
    ...
    # For Anthropic
    uv pip install kit[anthropic]
    
    # For Google
    uv pip install kit[google]
    
    # To install SDKs for all supported LLM providers
    uv pip install kit[all]
    ```
...
<Aside type="note">
  The first time you call `repo.get_summarizer()` (or instantiate `Summarizer` directly), it will attempt to import the necessary LLM library for the specified or default configuration (e.g., `openai`, `anthropic`, `google.generativeai`). If it's not installed, an `ImportError` will be raised, prompting you to install the required extras (e.g., `kit[openai]`, `kit[anthropic]`, `kit[google]`).
</Aside>
...
These summaries can also be embedded for semantic search – see [Docstring-Based Vector Index](/docs/core-concepts/docstring-indexing).
