...
1.  **Vector DB Backend**: Stores the actual embeddings and their metadata (like the summary text and source location). It defaults to using [`Chroma`](https://docs.trychroma.com/) and stores data in a subdirectory within the `persist_dir` (default: `.kit_cache/docstring_db/` inside your repo).
2.  **Cache Backend**: Stores metadata (like file hashes) to track changes and avoid re-processing unchanged files/symbols. It defaults to using `FilesystemCacheBackend`, storing a `meta.json` file in the `persist_dir`. You can configure alternative backends like `RedisCacheBackend` via the `cache_backend` parameter in the constructor.

These backends can be configured independently using the `backend` and `cache_backend` parameters of the `DocstringIndexer` constructor.
