...
llm_summarizer_config = OpenAIConfig(model="gpt-4o") # Or "gpt-4.1", etc.
...
<Aside type="tip">
  DocstringIndexer supports both local ChromaDB and Chroma Cloud backends. When `KIT_USE_CHROMA_CLOUD="true"` is set with appropriate credentials, your docstring indexes will be stored in the cloud automatically. See the [Semantic Search documentation](/core-concepts/semantic-search#chroma-cloud-managed-service) for Chroma Cloud configuration details.
</Aside>
