...
2.  **Building the Index**: Call `indexer.build_index()`.
    ...
    ```python
    indexer.build_index(file_extensions=[".py", ".md"])
    ```
...
To enable symbol-level indexing, pass `level="symbol"` to `build_index()`:

```python
# Build a symbol-level index
indexer.build_index(level="symbol", file_extensions=[".py"], force=True)
```
...
The quality of your search results will heavily depend on the embedding model
used.  Consider models like:
*   [Voyage AI](https://voyageai.com/) (good for code)
*   OpenAI `text-embedding-ada-002` or newer `text-embedding-3-small`/`large`
*   Sentence Transformers (various open-source models)

Remember to align the `embed_fn` you provide to `DocstringIndexer` with the
model you intend to use for querying with `SummarySearcher`.
