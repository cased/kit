...
export class Kit {
  ...
  async fileTree(path?: string, ref?: string): Promise<FileNode[]> {
    ...
    const output = await this.exec(args);
    return this.parseJSON<FileNode[]>(output);
  }

  /**
   * Search for text in files
   */
  async search(query: string, path?: string, options: SearchOptions = {}): Promise<SearchResult[]> {
  ...
  async searchSemantic(
    ...
    if (options.embeddingModel) args.push('--embedding-model', options.embeddingModel);
   ...
   */
  async usages(symbolName: string, path?: string, options: UsagesOptions = {}): Promise<any> {
   ...
   * Get dependencies
   */
  async dependencies(filePath: string, options: DependenciesOptions = {}): Promise<any> {
   ...
   */
  async reviewPR(prUrl: string, options: PRReviewOptions = {}): Promise<string> {
...
export class Repository {
  ...
  async symbols(filePath: string, options?: Omit<SymbolOptions, 'ref'>): Promise<Symbol[]> {
    return this.kit.symbols(`${this.path}/${filePath}`, { ...options, ref: this.ref });
  }
  ...
  async searchSemantic(query: string, options?: Omit<SemanticSearchOptions, 'ref'>): Promise<SemanticSearchResult[]> {
    return this.kit.searchSemantic(this.path, query, { ...options, ref: this.ref });
  }

  async usages(symbolName: string, options?: Omit<UsagesOptions, 'ref'>): Promise<any> {
    return this.kit.usages(symbolName, this.path, { ...options, ref: this.ref });
  }

  async dependencies(filePath: string, options?: Omit<DependenciesOptions, 'ref'>): Promise<any> {
    return this.kit.dependencies(`${this.path}/${filePath}`, { ...options, ref: this.ref });
  }
