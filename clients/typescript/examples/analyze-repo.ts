...
async function main() {
  ...
  const sourceFiles = files.filter(f => !f.is_dir && 
  ...
  for (const file of sourceFiles.slice(0, 3)) {
    ...
    if (functions.length > 0) {
      console.log(`  - Functions: ${functions.slice(0, 3).map(f => f.name).join(', ')}${functions.length > 3 ? '...' : ''}`);
    }
  }
  ...
  try {
    ...
    console.log('Semantic search not available (requires sentence-transformers)');
  }
}
