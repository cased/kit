...
class TestSearchCommands:
    ...
    def test_search_with_options(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["search", ".", "test", "--pattern", "*.py"])
    ...
    def test_search_json_output(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            mock_repo.return_value.search.return_value = [
                {"file": "test.py", "line": 1, "content": "test line"}
            ]
...
class TestGrepCommand:
    ...
    def test_grep_case_sensitivity(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["grep", ".", "TEST"])
            # Should search case-sensitively by default
            
        # Case insensitive
        with patch("kit.Repository") as mock_repo:
            mock_repo.return_value.grep.return_value = []
            result = runner.invoke(app, ["grep", ".", "TEST", "--ignore-case"])
...
class TestDependenciesCommand:
    ...
    def test_dependencies_visualization(self, runner):
        ...
        result = runner.invoke(app, ["dependencies", ".", "--language", "python", "--visualize", "output.png", "--help"])
        assert result.exit_code == 0
        
        # Test with different formats
        for fmt in ["png", "svg", "pdf"]:
            viz_file = f"output.{fmt}"
            result = runner.invoke(app, ["dependencies", ".", "--language", "python", "--visualize", viz_file, "--help"])
            assert result.exit_code == 0
...
class TestSymbolCommands:
    ...
    def test_symbols_formats(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            # JSON format
            result = runner.invoke(app, ["symbols", ".", "--format", "json"])
            
            # Names format
            result = runner.invoke(app, ["symbols", ".", "--format", "names"])
    ...
    def test_usages_with_type(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["usages", ".", "MyClass", "--type", "class"])
...
class TestContextCommands:
    ...
    def test_context_with_line_number(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["context", ".", "file.py", "42"])
            # Should execute without errors
    ...
    def test_chunk_lines_options(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["chunk-lines", ".", "file.py", "--max-lines", "100"])
            
            # With overlap
            result = runner.invoke(app, ["chunk-lines", ".", "file.py", "--overlap", "10"])
...
class TestExportCommand:
    ...
    def test_export_with_ref(self, runner):
        ...
        with patch("kit.Repository") as mock_repo:
            ...
            result = runner.invoke(app, ["export", ".", "symbols", "output.json", "--ref", "v1.0.0"])
