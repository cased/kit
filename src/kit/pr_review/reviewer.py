...
**Guidelines:** Be specific, actionable, and professional. Reference actual diff content. Focus on issues worth fixing. Use measured technical language - distinguish between defensive code (with safeguards) and actual vulnerabilities."""
    ...
    async def _analyze_with_anthropic_enhanced(self, enhanced_prompt: str) -> str:
        ...
        try:
            # Run the blocking HTTP request in a background thread so the event loop stays responsive.
            # The Anthropic SDK doesn't expose an async interface yet, so we off-load it via asyncio.to_thread().
            # We also pass a conservative timeout (default 90 s) to prevent indefinite hangs when the API or
            # network misbehaves.
