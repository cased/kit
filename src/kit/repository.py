...
class Repository:
    ...
    def _checkout_ref(self, ref: str) -> None:
        ...
        try:
            ...
            # Checkout the ref
            _ = subprocess.run(
                ["git", "checkout", ref], cwd=str(self.local_path), capture_output=True, text=True, encoding='utf-8', check=True
            )
    ...
    def _git_command(self, cmd: List[str]) -> Optional[str]:
        ...
        try:
            result = subprocess.run(cmd, cwd=self.repo_path, capture_output=True, text=True, encoding='utf-8', check=True)
            ...
            if ref:
                try:
                    current_sha = subprocess.run(
                        ["git", "rev-parse", "HEAD"], cwd=str(repo_path), capture_output=True, text=True, encoding='utf-8', check=True
                    ).stdout.strip()

                    # Check if we're already on the requested ref
                    target_sha = subprocess.run(
                        ["git", "rev-parse", ref], cwd=str(repo_path), capture_output=True, text=True, encoding='utf-8', check=False
                    )
